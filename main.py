...
def is_admins(chat_id: int):
    return [
        member.user.id
        for member in bot.iter_chat_members(
...
def chatbotofd(client, message):
    ...
    if not is_vick:
        ...
        message.reply_text(f"Chatbot Disabled!")
    if is_vick:
        message.reply_text(f"ChatBot Is Already Disabled")
    

@bot.on_message(
    filters.command("on", prefixes=["/", ".", "?", "-"])
    & ~filters.private)
def chatboton(client, message):
    vickdb = MongoClient(MONGO_URL)    
    vick = vickdb["VickDb"]["Vick"]     
    if message.from_user:
        user = message.from_user.id
    ...
    is_vick = vick.find_one({"chat_id": message.chat.id})
    if not is_vick:           
        message.reply_text(f" Already Enabled")
    if is_vick:
        vick.delete_one({"chat_id": message.chat.id})
        message.reply_text(f" Enable!")
...
def vickai(client: Client, message: Message):
